...
def sample_same_tag(
    ...
    val_data = datasets[tag].data.iloc[_N_TEST:]
...
def get_prompt(
    ...
    tag: str,
    n_shots: int = 0,
    sample_examples: Callable[[str, str, int], Iterator[Sequence[Any]]] = sample_same_tag,
    process_example: Callable[[Sequence[Any]], str] = process_example,
    process_question: Callable[[str], str] = process_question_vanilla,
) -> str:
    prompt = ''
    for example in sample_examples(curr_question, tag, n_shots):
...
def get_response(
    ...
    prompt = get_prompt(question, tag, n_shots)
...
def query_model(
    ...
    for _, row in tqdm(data.iterrows()):
        yield get_response(
            model, row[_QUESTION_KEY], tag, n_shots, request_func)
...
def save_model_results(
    ...
    prompt_response_pairs = query_model(
        model, tag, n_shots, request_func, indices, first_n)
    data = get_data_slice(tag, indices, first_n)
...
if __name__ == '__main__':
    ...
    for tag in ARITHMETIC_DATASET_TAGS:
        for n_shots in range(4):
            first_n = 100
            path = f'{_RESULT_DIRECTORY}/{model_name}_{tag}_{n_shots}_{first_n}.jsonl'
